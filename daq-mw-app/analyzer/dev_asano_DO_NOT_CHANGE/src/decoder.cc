...
decodeTdcWord(unsigned int v,
              ...
              unsigned int& ft)
{
  unsigned int data_type = (v >> Decoder::ktdc_data_type_shift) & Decoder::ktdc_data_type_mask;
  ct = (v >> Decoder::ktdc_coarse_time_shift) & Decoder::ktdc_coarse_time_mask;
  ft = (v >> Decoder::ktdc_fine_time_shift) & Decoder::ktdc_fine_time_mask;
  return data_type;
}
...
decodeTdcWord(unsigned int v,
              ...
              unsigned int& ft)
{
  ch = (v >> Decoder::ktdc_ch_id_shift) & Decoder::ktdc_ch_id_mask;
...
Decoder::getNextEvent()
{
  ...
  for(int imod=0;imod<2;imod++){
    ...
    if(flag[imod][EoF]) return false;

    int n_word_body   = head_buffer_[imod][i_datasize] & data_size_mask;
    if(flag[imod][FirstData]){
      int n_word_header = (head_buffer_[imod][i_datasize] >> header_size_shift) & header_size_mask;
    ...
    { // TDC data read header
      tdc_buffer_[imod].resize(sizeTdcHeader, 0);
      ...
      for (;;) {
        ...
        unsigned int d32 = tdc_buffer_[imod][i_frame_size];
...
Decoder::decode(dataDrs& cont)
{
  ...
  // decode base header
  for(int imod=0;imod<NofBoards;imod++){
    cont.nword_header[imod] = (head_buffer_[imod][i_datasize] >> header_size_shift) & header_size_mask;
    cont.nword_body[imod] =  head_buffer_[imod][i_datasize] & data_size_mask;
    cont.global_tag[imod]    = (head_buffer_[imod][i_tag] >> gtag_shift) & gtag_mask;
    cont.local_tag[imod]      =  head_buffer_[imod][i_tag] & ltag_mask;
    cont.tic_count[imod]    =  head_buffer_[imod][i_tic] & tic_mask;
    cont.fl_double_data[imod] = (head_buffer_[imod][i_tic] >> datastr_shift ) & datastr_mask;
    ...
    if(flag[imod][ExHead]){
      ...
      for(unsigned int i = 0; i<sizeExHeader; ++i){
        int icel = i + imod*NofDrs;
        cont.wsr[icel]     = (exhead_buffer_[imod][icel] >> wsr_shift) & wsr_mask;
        cont.cellnum[icel] =  exhead_buffer_[imod][icel] & cellnum_mask;
      }// for(i)
    }// if(flag)
    ...
    for(int i = 0; i<cont.nword_body[imod]; ++i){
      ...
      unsigned int data_type = (body_buffer_[imod][i] >> type_shift) & type_mask;
      unsigned int ch        = (body_buffer_[imod][i] >> ch_shift) & ch_mask;
      int ich = ch + imod*NofChModule;
      if(data_type == type_wf){
        if(cont.fl_double_data){
          double data_high = (double)((body_buffer_[imod][i] >> wf_high_shift) & wf_mask_12bit);
          double data_low  = (double)(body_buffer_[imod][i] & wf_mask_12bit);
          cont.data_wf[ich].push_back(data_low);
          cont.data_wf[ich].push_back(data_high);
        }else{
          unsigned int data  = (body_buffer_[imod][i] & wf_mask_14bit);
...
#if BLCOR
        int drs4_id = (int)ich/4;
        data_low  -= getCorBL(drs4_id, current_cellnum[ich]++);
        if(current_cellnum[ich] == 1024) current_cellnum[ich] = 0;
        data_high -= getCorBL(drs4_id, current_cellnum[ich]++);
        if(current_cellnum[ich] == 1024) current_cellnum[ich] = 0;
#endif

      }else if(data_type == type_qdc){
        unsigned int data_qdc = body_buffer_[imod][i] & qdc_mask;
...
//______________________________________________________________________________
bool
Decoder::decodeTDC(dataDrs& cont)
{
  ...
  for(int imod = 0 ; imod <2 ;imod++){
    ...
    unsigned int l1_traw  = tdc_buffer_[imod][i_l1_tdc];
    unsigned int l1_t1raw = tdc_buffer_[imod][i_l1_tdc_2nd];
    {
      unsigned int ct;
      unsigned int ft;
    ...
    for (int i=sizeTdcHeader; i<n; ++i) {
      ...
      unsigned int ich = ch + imod*NofChModule;
...
//______________________________________________________________________________
bool
Decoder::decodeADC(dataDrs& cont)
{
...
// Constructor ---------------------------------------------------------------
Decoder::Decoder()//C
  //head_buffer_[0](sizeBaseHeader),
  //exhead_buffer_[0](sizeExHeader),
  //body_buffer_[0](1),
  //head_buffer_[1](sizeBaseHeader),
  //exhead_buffer_[1](sizeExHeader),
  //body_buffer_[1](1)
{
  for(int imod=0;imod<2;imod++){
...
Decoder::setThisEvent(int tic, bool of)
{
  for(int imod=0;imod<NofBoards;imod++){
    for(int i = 0; i<NofDrs; ++i){
      for(int cell = 0; cell<NofCell; ++cell){
        if(of){
          contTic[i][imod][cell].time_interval = max_tic;
          contTic[i][imod][cell].fl_overflow   = true;	
        }else{
          if(!contTic[i][imod][cell].fl_overflow){
            contTic[i][imod][cell].time_interval += tic;
            if(contTic[i][imod][cell].time_interval > max_tic){
              contTic[i][imod][cell].time_interval = max_tic;
              contTic[i][imod][cell].fl_overflow   = true;
            }
          }// if(fl_overflow)
...
/*
double
Decoder::getCorBL(int drs, int cellnum)
{
  
  // set fl_read
  contTic[drs][imod][cellnum].fl_read = true;

  // no correction becase of over flow
  if(contTic[drs][imod][cellnum].fl_overflow) return 0;
...
#if VER2
  double xmean;
  int ix = (int)contTic[drs][imod][cellnum].time_interval;
  if((ix >> 14) != 0) ix = 10000;
  for(int i = 0; i<2; ++i){
  ...
  cor  = (int)cor;
#else
  double x = log(contTic[drs][imod][cellnum].time_interval);
  double cor = (pa*x*x + pb*x + pc - baseline);
#endif
...
int Decoder::getsublayer(int ch=-1)
{
  if(64<= ch) ch -= 64;
...
int Decoder::geteasiroc(int ch=-1)
{
  int easiroc = -1;
