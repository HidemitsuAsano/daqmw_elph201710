...
private:
  ...
  bufType head_buffer_[2];
  bufType exhead_buffer_[2];
  bufType body_buffer_[2];
  bufType tdc_buffer_[2];
  int modid_;

  
  enum Flags{ Good, EoF, ExHead, FirstData, DoubleData,
	      sizeFlags};
  std::bitset<sizeFlags> flag[2];
  ...
  ticType contTic[4][2]; // for each DRS4 chip
  ...
  //NER 
  int getXUV(int ch);
  int getsublayer(int ch);
  int getlayer(int ch);
  int getfiber(int ch);
  int geteasiroc(int ch);
