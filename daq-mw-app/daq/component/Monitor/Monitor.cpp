...
const int nNERch = 128;
std::vector<TH1*> m_adcHigh(nNERch);
std::vector<TH1*> m_TOT(nNERch);
...
{
  for (auto&& p : m_adcHigh) if (p) p->Reset();
  for (auto&& p : m_TOT)     if (p) p->Reset();
}
