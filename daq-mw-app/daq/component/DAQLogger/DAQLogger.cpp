...
int DAQLogger::daq_start()
{
    ...
    std::cout << "start time " << fileUtils->get_date_time() << std::endl;
    m_in_status = BUF_SUCCESS;
    m_filesOpened = false;
...
int DAQLogger::daq_run()
{
    ...
    if(m_debug){
      std::cerr << "read InPort.. " << ret << std::endl;
    }
    ...
    if (ret == true) {
        ...
        if (m_debug) {
            std::cerr << "m_in_data.data.length:"
                      << m_in_data.data.length() << std::endl;
            std::cerr << "event_byte_size w/o header, footer = "
                      << event_byte_size << std::endl;
        }

        if (event_byte_size == 0) {
            return 0;
        }else if(event_byte_size > 0 ){
           if(m_debug){
             std::cerr << "veto clear " << std::endl;
           }
           //m_v1718->OnePulse();
        }
      ...
      if(m_debug){
        std::cerr << "L." << __LINE__ << " do nothing " << std::endl;
      }
      return 0;
    ...
    if (m_isDataLogging) {
        ...
        //                                event_byte_size);
        int ret = fileUtils->write_data((char *)&m_in_data.data[0],
                                        m_in_data.data.length());
