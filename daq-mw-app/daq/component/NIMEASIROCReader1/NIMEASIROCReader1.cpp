...
{
    "implementation_id", "NIMEASIROCReader1",
    "type_name",         "NIMEASIROCReader1",
    "description",       "NIMEASIROCReader1 component",
    "version",           "1.0",
...
NIMEASIROCReader1::NIMEASIROCReader1(RTC::Manager* manager)
      ...
      m_debug(0)
{
    ...
    init_command_port();
    init_state_table();
    set_comp_name("NIMEASIROCReader1");
}

NIMEASIROCReader1::~NIMEASIROCReader1()
{
}

RTC::ReturnCode_t NIMEASIROCReader1::onInitialize()
{
    if (m_debug) {
        std::cerr << "NIMEASIROCReader1::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NIMEASIROCReader1::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NIMEASIROCReader1::daq_dummy()
{
    return 0;
}

int NIMEASIROCReader1::daq_configure()
{
    std::cerr << "*** NIMEASIROCReader1::configure" << std::endl;
...
int NIMEASIROCReader1::parse_params(::NVList* list)
{
...
int NIMEASIROCReader1::daq_unconfigure()
{
    std::cerr << "*** NIMEASIROCReader1::unconfigure" << std::endl;
    //exit DAQ mode
    //MonitorMode();
    if(m_rbcp){
      delete m_rbcp;
...
int NIMEASIROCReader1::daq_start()
{

    //std::cerr << "*** NIMEASIROCReader1::start" << std::endl;


    return 0;
}

int NIMEASIROCReader1::daq_stop()
{
    std::cerr << "*** NIMEASIROCReader1::stop" << std::endl;
...
int NIMEASIROCReader1::daq_pause()
{
    std::cerr << "*** NIMEASIROCReader1::pause" << std::endl;

    return 0;
}

int NIMEASIROCReader1::daq_resume()
{
    std::cerr << "*** NIMEASIROCReader1::resume" << std::endl;

    return 0;
}

int NIMEASIROCReader1::read_data_from_detectors()
{
...
int NIMEASIROCReader1::set_data(unsigned int data_byte_size)
{
...
int NIMEASIROCReader1::write_OutPort()
{
...
int NIMEASIROCReader1::daq_run()
{
    if (m_debug) {
        std::cerr << "*** NIMEASIROCReader1::run" << std::endl;
    }
...
void NIMEASIROCReader1::DaqMode()
{
...
//convert 4 char arrays to a single 32 bit word with 32-bit BigEndian format
unsigned int NIMEASIROCReader1::unpackBigEndian32(const unsigned char* array4byte)
{
...
unsigned int NIMEASIROCReader1::Decode32bitWord(unsigned int word32bit)
{
  //check data format
...
bool NIMEASIROCReader1::datacheck(std::vector <unsigned char> data)
{
...
extern "C"
{
    void NIMEASIROCReader1Init(RTC::Manager* manager)
    {
        ...
        manager->registerFactory(profile,
                    RTC::Create<NIMEASIROCReader1>,
                    RTC::Delete<NIMEASIROCReader1>);
    }
};
