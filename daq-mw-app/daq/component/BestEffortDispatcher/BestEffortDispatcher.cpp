...
int BestEffortDispatcher::daq_configure()
{
  ...
  //scaler stop
  m_v1718->SetBusy(0);
  //generate latch from output line 2 (AS)
  //m_v1718->SetBusy2(1);
  m_v1718->OnePulse();
...
unsigned int BestEffortDispatcher::read_InPort0()
{
  ...
  if (ret == false) { // false: TIMEOUT or FATAL
    ...
    if (m_Verbosity>10) {
      std::cout << "tout_0  " << m_in_tout_counts[0] << std::endl;
    }
...
unsigned int BestEffortDispatcher::read_InPort1()
{
  ...
  if (ret == false) { // false: TIMEOUT or FATAL
    ...
    if (m_Verbosity>10) {
      std::cout << "tout_1  " << m_in_tout_counts[1] << std::endl;
    }
...
unsigned int BestEffortDispatcher::read_InPort2()
{
  ...
  if (ret == false) { // false: TIMEOUT or FATAL
    ...
    if (m_Verbosity>10) {
      std::cout << "tout_2  " << m_in_tout_counts[2] << std::endl;
    }
...
unsigned int BestEffortDispatcher::read_InPort3()
{
  ...
  if (ret == false) { // false: TIMEOUT or FATAL
    ...
    if (m_Verbosity>10) {
      std::cout << "tout_3  " << m_in_tout_counts[3] << std::endl;
    }
...
unsigned int BestEffortDispatcher::read_InPort4()
{
  ...
  if (ret == false) { // false: TIMEOUT or FATAL
    ...
    if (m_Verbosity>10) {
      std::cout << "tout_4  " << m_in_tout_counts[4] << std::endl;
    }
  ...
  //memcpy(&header[0],&m_in4_data.data[HEADER_BYTE_SIZE],3*sizeof(int));
  /*
  std::vector<unsigned int> recv_data;
  recv_data.resize(3);
  memcpy(reinterpret_cast<unsigned char*>(&recv_data[0]),
         &m_in4_data.data[HEADER_BYTE_SIZE],
         3*sizeof(int));   

  //unsigned int n_word_data = header[1] & 0x3ff;
  unsigned int n_word_data = recv_data.at(1) & 0x3ff;
  unsigned int sizeData    = n_word_data*sizeof(unsigned int);
  if(sizeData ==0){  
    recv_byte_size = 0;
  }*/
  if(m_Verbosity){
    std::cout << "HUL data size " << recv_byte_size << std::endl;
  }
...
int BestEffortDispatcher::daq_run()
{
  ...
  if (m_out_status != BUF_TIMEOUT) {
    if(m_inport_recv_data_size[0]==0) m_inport_recv_data_size[0] = read_InPort0();
    if(m_inport_recv_data_size[1]==0) m_inport_recv_data_size[1] = read_InPort1();
    if(m_inport_recv_data_size[2]==0) m_inport_recv_data_size[2] = read_InPort2();
    if(m_inport_recv_data_size[3]==0) m_inport_recv_data_size[3] = read_InPort3();
    if(m_inport_recv_data_size[4]==0) m_inport_recv_data_size[4] = read_InPort4();
    ...
    if ((m_inport_recv_data_size[0] == 0)
     ...
     || (m_inport_recv_data_size[4] == 0)  
      ...
      for(int imod=0;imod<NINMODULE;imod++){
        m_inport_recv_data_size[imod] = 0;
      }
    }//if not time out
  }else{
    std::cerr << "out_status TIME OUT "  << std::endl; 
  }
