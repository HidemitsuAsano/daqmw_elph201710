// -*- C++ -*-
/*!
 * @file
 * @brief
 * @date
 * @author
 *
 */

#include "Dispatcher.h"

using DAQMW::FatalType::DATAPATH_DISCONNECTED;
using DAQMW::FatalType::INPORT_ERROR;
using DAQMW::FatalType::OUTPORT_ERROR;
using DAQMW::FatalType::USER_DEFINED_ERROR1;

// Module specification
// Change following items to suit your component's spec.
static const char* dispatcher_spec[] =
{
    "implementation_id", "Dispatcher",
    "type_name",         "Dispatcher",
    "description",       "Dispatcher component",
    "version",           "1.0",
    "vendor",            "Kazuo Nakayoshi, KEK",
    "category",          "example",
    "activity_type",     "DataFlowComponent",
    "max_instance",      "1",
    "language",          "C++",
    "lang_type",         "compile",
    ""
};

Dispatcher::Dispatcher(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort0("dispatcher_in0", m_in0_data),
      m_InPort1("dispatcher_in1", m_in1_data),
      m_OutPort0("dispatcher_out0", m_out0_data),
      m_OutPort1("dispatcher_out1", m_out1_data),
      m_in0_status(BUF_SUCCESS),
      m_in1_status(BUF_SUCCESS),
      m_out0_status(BUF_SUCCESS),
      m_out1_status(BUF_SUCCESS),
      //m_canvas(NULL),
      //m_hist(NULL),
      m_in0_timeout_counter(0),
      m_in1_timeout_counter(0),
      m_out0_timeout_counter(0),
      m_out1_timeout_counter(0),
      m_inport_recv_data_size_sum(0),
      m_inport0_recv_data_size(0),
      m_inport1_recv_data_size(0),
      m_debug(false)
{
    // Registration: InPort/OutPort/Service

    // Set OutPort buffers
    registerInPort("dispatcher_in0", m_InPort0);
    registerInPort("dispatcher_in1", m_InPort1);
    registerOutPort("dispatcher_out0", m_OutPort0);
    registerOutPort("dispatcher_out1", m_OutPort1);

    init_command_port();
    init_state_table();
    set_comp_name("DISPATCHER");
}

Dispatcher::~Dispatcher()
{
}

RTC::ReturnCode_t Dispatcher::onInitialize()
{
    if (m_debug) {
        std::cerr << "Dispatcher::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t Dispatcher::onExecute(RTC::UniqueId ec_id)
{
    daq_do();

    return RTC::RTC_OK;
}

int Dispatcher::daq_dummy()
{
    /*
    if (m_canvas) {
        m_canvas->Update();
        // daq_dummy() will be invoked again after 10 msec.
        // This sleep reduces X servers' load.
        sleep(1);
    }*/

    return 0;
}

int Dispatcher::daq_configure()
{
    std::cerr << "*** Dispatcher::configure" << std::endl;

    ::NVList* paramList;
    paramList = m_daq_service0.getCompParams();
    parse_params(paramList);

    return 0;
}

int Dispatcher::parse_params(::NVList* list)
{
    std::cerr << "param list length:" << (*list).length() << std::endl;

    int len = (*list).length();
    for (int i = 0; i < len; i+=2) {
        std::string sname  = (std::string)(*list)[i].value;
        std::string svalue = (std::string)(*list)[i+1].value;

        std::cerr << "sname: " << sname << "  ";
        std::cerr << "value: " << svalue << std::endl;
    }

    return 0;
}

int Dispatcher::daq_unconfigure()
{
    std::cerr << "*** Dispatcher::unconfigure" << std::endl;

    return 0;
}

int Dispatcher::daq_start()
{
    std::cerr << "*** Dispatcher::start" << std::endl;
    m_in0_status   = BUF_SUCCESS;
    m_in1_status   = BUF_SUCCESS;
    m_out0_status = BUF_SUCCESS;
    m_out1_status = BUF_SUCCESS;


    /*
    if (m_canvas) {
        delete m_canvas;
        m_canvas = NULL;
    }
    m_canvas = new TCanvas("c1", "DAQ status", 1000, 0, 600, 400);
    

    gStyle->SetOptStat("em");
    m_hist = new TH1I("daq_status","DAQ status",4,0,4);
    */
    return 0;
}

int Dispatcher::daq_stop()
{
    std::cerr << "*** Dispatcher::stop" << std::endl;
    reset_InPort0();
    reset_InPort1();
    std::cerr << "*** reset InPorts.. " << std::endl;
    std::cout << "total sequence # " << get_sequence_num() << std::endl;
    return 0;
}

int Dispatcher::daq_pause()
{
    std::cerr << "*** Dispatcher::pause" << std::endl;

    return 0;
}

int Dispatcher::daq_resume()
{
    std::cerr << "*** Dispatcher::resume" << std::endl;

    return 0;
}

int Dispatcher::read_data_from_detectors()
{
    int received_data_size = 0;
    /// write your logic here
    return received_data_size;
}

int Dispatcher::set_data_OutPort0(unsigned int data_byte_size)
{
    ///set OutPort buffer length
    m_out0_data.data.length(data_byte_size);
    memcpy(&(m_out0_data.data[0]), &m_in_data_sum.data[0], data_byte_size);
    return 0;
}

int Dispatcher::set_data_OutPort1(unsigned int data_byte_size)
{
    ///set OutPort buffer length
    m_out1_data.data.length(data_byte_size);
    memcpy(&(m_out1_data.data[0]), &(m_in_data_sum.data[0]), data_byte_size);

    return 0;
}

int Dispatcher::reset_InPort0()
{
    TimedOctetSeq dummy_data;

    while (m_InPort0.isEmpty() == false) {
        m_InPort1 >> dummy_data;
    }
    //std::cerr << "*** Dispatcher::InPort flushed\n";
    return 0;
}

int Dispatcher::reset_InPort1()
{
    TimedOctetSeq dummy_data;

    while (m_InPort1.isEmpty() == false) {
        m_InPort1 >> dummy_data;
    }
    //std::cerr << "*** Dispatcher::InPort flushed\n";
    return 0;
}

unsigned int Dispatcher::read_InPort0()
{
    /////////////// read data from InPort Buffer ///////////////
    unsigned int recv_byte_size = 0;
    bool ret = m_InPort0.read();

    //////////////////// check read status /////////////////////
    if (ret == false) { // false: TIMEOUT or FATAL
        m_in0_status = check_inPort_status(m_InPort0);
        if (m_in0_status == BUF_TIMEOUT) { // Buffer empty.
            m_in0_timeout_counter++;
            if (check_trans_lock()) {     // Check if stop command has come.
                set_trans_unlock();       // Transit to CONFIGURE state.
            }
        }
        else if (m_in0_status == BUF_FATAL) { // Fatal error
            fatal_error_report(INPORT_ERROR);
        }
    }
    else { // success
        m_in0_timeout_counter = 0;
        recv_byte_size = m_in0_data.data.length();
        m_in0_status = BUF_SUCCESS;
    }
    if (m_debug) {
        std::cerr << "m_in0_data.data.length():" << recv_byte_size
                  << std::endl;
    }

    return recv_byte_size;
}

unsigned int Dispatcher::read_InPort1()
{
    /////////////// read data from InPort Buffer ///////////////
    unsigned int recv_byte_size = 0;
    bool ret = m_InPort1.read();

    //////////////////// check read status /////////////////////
    if (ret == false) { // false: TIMEOUT or FATAL
        m_in1_status = check_inPort_status(m_InPort1);
        if (m_in1_status == BUF_TIMEOUT) { // Buffer empty.
            m_in1_timeout_counter++;
            if (check_trans_lock()) {     // Check if stop command has come.
                set_trans_unlock();       // Transit to CONFIGURE state.
            }
        }
        else if (m_in1_status == BUF_FATAL) { // Fatal error
            fatal_error_report(INPORT_ERROR);
        }
    }
    else { // success
        m_in1_timeout_counter = 0;
        recv_byte_size = m_in1_data.data.length();
        m_in1_status = BUF_SUCCESS;
    }
    if (m_debug) {
        std::cerr << "m_in1_data.data.length():" << recv_byte_size
                  << std::endl;
    }

    return recv_byte_size;
}

int Dispatcher::write_OutPort0()
{
    ////////////////// send data from OutPort  //////////////////
    bool ret = m_OutPort0.write();

    //////////////////// check write status /////////////////////
    if (ret == false) {  // TIMEOUT or FATAL
        m_out0_status  = check_outPort_status(m_OutPort0);
        if (m_out0_status == BUF_FATAL) {   // Fatal error
            fatal_error_report(OUTPORT_ERROR);
        }
        if (m_out0_status == BUF_TIMEOUT) { // Timeout
            if (check_trans_lock()) {     // Check if stop command has come.
                set_trans_unlock();       // Transit to CONFIGURE state.
            }
            m_out0_timeout_counter++;
            return -1;
        }
    }
    else { // success
        m_out0_timeout_counter = 0;
    }
    return 0; // successfully done
}


//change to best effort dispatcher
int Dispatcher::write_OutPort1()
{
    ////////////////// send data from OutPort  //////////////////
    bool ret = m_OutPort1.write();

    //////////////////// check write status /////////////////////
    if (ret == false) {  // TIMEOUT or FATAL
        m_out1_status  = check_outPort_status(m_OutPort1);
        if (m_out1_status == BUF_FATAL) {   // Fatal error
            //fatal_error_report(OUTPORT_ERROR);
        }
        if (m_out1_status == BUF_TIMEOUT) { // Timeout
            //if (check_trans_lock()) {     // Check if stop command has come.
            //    set_trans_unlock();       // Transit to CONFIGURE state.
            //}
            m_out1_timeout_counter++;
            return -1;
        }
    }
    //else { // success
    m_out1_timeout_counter = 0;
    //}
    return 0; // successfully done
}

int Dispatcher::daq_run()
{
    if (m_debug) {
        std::cerr << "*** Dispatcher::run" << std::endl;
    }

//    if ((m_out0_status != BUF_TIMEOUT) && (m_out1_status != BUF_TIMEOUT)) {
    if ((m_out0_status != BUF_TIMEOUT)) {
        m_inport0_recv_data_size = read_InPort0();
        m_inport1_recv_data_size = read_InPort1();

        if(m_inport0_recv_data_size == 0){ //TIMEOUT or empty
            if (m_debug) {
              std::cerr << "L." << __LINE__ << "inport0 timeout " << std::endl;
              //m_hist->Fill(1);
            }
            return 0;
        }else if(m_inport1_recv_data_size == 0){ //TIMEOUT or empty
            if (m_debug) {
              std::cerr << "L." << __LINE__ << "inport1 timeout " << std::endl;
              //m_hist->Fill(2);
            }
            return 0;
        }else {
            //m_hist->Fill(3);
            //check_header_footer(m_in0_data, m_inport0_recv_data_size);
            //check_header_footer(m_in1_data, m_inport1_recv_data_size);
            //take sum of inport0 and inport1
            //total data size =
            //header0 + data_body0 + footer0 + header1 + data_body1 + footer1
            //
            //here, footer0 and header1 are removed
            m_inport_recv_data_size_sum = m_inport0_recv_data_size + m_inport1_recv_data_size - HEADER_BYTE_SIZE - FOOTER_BYTE_SIZE;
            unsigned int data_size_body = m_inport_recv_data_size_sum - HEADER_BYTE_SIZE - FOOTER_BYTE_SIZE;
            unsigned char newheader[8];
            unsigned char newfooter[8];
            set_header(&newheader[0],data_size_body);
            set_footer(&newfooter[0]);
            //m_data_sum.clear();

            //m_data_sum.resize(m_inport_recv_data_size_sum);

            //for(int idata = 0; idata < NIMEASIROC::headersize; idata++){
             // m_data_sum.at(idata) = m_header[idata];
            //}
            if(m_inport_recv_data_size_sum > SEND_BUFFER_SIZE){
              std::cerr << "buffer overflow ! sizesum " << m_inport_recv_data_size_sum << std::endl;
            }
            //set Outpor buffer length
            m_in_data_sum.data.length(m_inport_recv_data_size_sum);
            //memset(&m_in_data_sum.data,0,sizeof(&m_in_data_sum.data));
            //set new header with corrected data size
            memcpy(&m_in_data_sum.data[0],&newheader[0],HEADER_BYTE_SIZE);
            //set data0
            memcpy(&m_in_data_sum.data[HEADER_BYTE_SIZE],&m_in0_data.data[HEADER_BYTE_SIZE],m_inport0_recv_data_size-FOOTER_BYTE_SIZE);
            //set data1
            memcpy(&m_in_data_sum.data[m_inport0_recv_data_size-FOOTER_BYTE_SIZE],&m_in1_data.data[HEADER_BYTE_SIZE],m_inport1_recv_data_size-HEADER_BYTE_SIZE-FOOTER_BYTE_SIZE);
            //set new footer
            memcpy(&m_in_data_sum.data[m_inport_recv_data_size_sum-FOOTER_BYTE_SIZE],&newfooter[0],FOOTER_BYTE_SIZE);
            
            set_data_OutPort0(m_inport_recv_data_size_sum);
            set_data_OutPort1(m_inport_recv_data_size_sum);
        }
    }

    if ((m_in0_status != BUF_TIMEOUT) && (m_in1_status != BUF_TIMEOUT) ) {
        if (write_OutPort0() < 0) { // TIMEOUT
          if (m_debug) {
            std::cerr << "L." << __LINE__ << "write_outport0 timeout " << std::endl;
          }
            ; // do nothing
        }
        else {
            m_out0_status = BUF_SUCCESS;
        }
    }

    if ((m_in0_status != BUF_TIMEOUT) && (m_in1_status != BUF_TIMEOUT) && (m_out0_status != BUF_TIMEOUT)) {
        if (write_OutPort1() < 0) { // TIMEOUT
          if (m_debug) {
            std::cerr << "L." << __LINE__ << "write_outport1 timeout " << std::endl;
          }
            ; // do nothing
        }
        else {
            m_out1_status = BUF_SUCCESS;
        }
    }

    if ((m_in0_status  != BUF_TIMEOUT) &&
        (m_in1_status  != BUF_TIMEOUT) &&  
        (m_out0_status != BUF_TIMEOUT) ){
       // (m_out1_status != BUF_TIMEOUT)) {
        inc_sequence_num();                    // increase sequence num.
        unsigned int event_data_size = get_event_size(m_inport_recv_data_size_sum);
        inc_total_data_size(event_data_size);  // increase total data byte size
    }

    //m_hist->Draw();
    //m_canvas->Update();
    
    return 0;
}

extern "C"
{
    void DispatcherInit(RTC::Manager* manager)
    {
        RTC::Properties profile(dispatcher_spec);
        manager->registerFactory(profile,
                    RTC::Create<Dispatcher>,
                    RTC::Delete<Dispatcher>);
    }
};
