...
CVErrorCodes CAENv1718::OnePulse(CVPulserSelect PulSel)
{
    ...
    CVErrorCodes ret;
    ret = SetPulserConf(cvPulserA,2,1,cvUnit25ns,1);
    ret = CAENVME_StartPulser(*pHandle,PulSel);
    if(ret != cvSuccess)
...
//added by H.Asano
CVErrorCodes CAENv1718::OnePulse2(CVPulserSelect PulSel)
{
    SetOutputConf(cvOutput1,cvDirect,cvActiveHigh,cvMiscSignals);
    
    CVErrorCodes ret;
    ret = SetPulserConf(cvPulserB,2,1,cvUnit25ns,1);
    ret = CAENVME_StartPulser(*pHandle,PulSel);
    if(ret != cvSuccess)
        std::cout << "\n\n Error !!! [CAENv1718::OnePulse]\n" << std::endl;
...
CVErrorCodes CAENv1718::SetBusy2(int i)
{
    SetOutputConf(cvOutput2,cvDirect,cvActiveHigh,cvManualSW);
    switch(i){
        case 0 : 
            ClearOutputRegister(cvOut2Bit);
            break;
        case 1 :
            SetOutputRegister(cvOut2Bit);
            break;
        default :
            break;
    }
